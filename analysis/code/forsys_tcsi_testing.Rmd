...
input_operability <- raster('../data/TCSI/operability_class_with_scA_final.tif')
input_spottedowlhabitat <- raster('../data/TCSI/cso_habitat_18Oct2021.tif')
...
geo_clustering <- function(
  ...
  data <- df %>%
    # Removing unneccessary columns and rows with missing data
    tibble() %>%
...
```{r optimize_geo_weight}
# Creating a parameter tuning grid
x <- tibble(
  cluster_weight_geo = rep(c(.1, .5, 1, 2, 3, 5, 10, 20, 50, 100), 3),
  cluster_size = c(rep(50, 10), rep(100, 10), rep(200, 10)),
  cluster_variables = list(cluster_variables),
  df = list(planning_polygon)) %>%
  pmap_dfc(geo_clustering)
...
x2 <- tibble(
  ...
  cluster_variables = list(cluster_variables)) %>%
  mutate(clusters = x) %>%
  mutate(sc_prop = map_dbl(clusters, calculate_multipart_proportion))

ggplot(x2, aes(x = cluster_weight_geo, y = sc_prop, color = cluster_size, group = cluster_size)) + geom_line()
