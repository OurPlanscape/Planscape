...
class PillarConfigTest(unittest.TestCase):
    ...
    def test_get_region(self):
        region: Optional[Region] = self._config.get_region("sierra-nevada")
    ...
    def test_get_min_max_values(self):
        self.assertEqual(self._config.get_min_max_values("fire_dynamics.tif"), (-1, 1))
        self.assertEqual(
            self._config.get_min_max_values("functional_fire.tif"), (-1, 1)
        )
        self.assertEqual(
            self._config.get_min_max_values("TPA_30in_up_2021_300m.tif"), (0, 35)
        )
        self.assertEqual(
            self._config.get_min_max_values("TPA_30in_up_2021_300m_normalized.tif"),
            (-1, 1),
        )

    def test_get_data_units(self):
        self.assertEqual(self._config.get_data_units("fire_dynamics.tif"), None)
        self.assertEqual(self._config.get_data_units("functional_fire.tif"), None)
        self.assertEqual(
            self._config.get_data_units("TPA_30in_up_2021_300m.tif"), "Live Trees/Acre"
        )
        self.assertEqual(
            self._config.get_data_units("TPA_30in_up_2021_300m_normalized.tif"), None
        )
