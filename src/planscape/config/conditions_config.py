...
class PillarConfig:
    ...
    COMMON_METADATA = {
        ...
        "layer",
        "normalized_layer",
        "normalized_data_download_path",
    }
    REGION_KEYS = {
        "region_name",
        "pillars",
        "region_geoserver_name",
        "raw_data",
        "translated_data",
        "future_data",
    ...
    ELEMENT_KEYS = {"element_name", "metrics", "operation", "display"}.union(
        COMMON_METADATA
    )
    METRIC_KEYS = {
        "metric_name",
    ...
    def build_condition_metadata(cls, config: list[Region]):
        ...
        for region in config:
            for pillar in region["pillars"]:
                update_metadata(
                    pillar["pillar_name"],
                    pillar.get("filepath", None),
                    pillar.get("raw_layer", None),
                    pillar.get("normalized_layer", None),
                    pillar.get("future_layer", None),
                    -1,
                    1,
                    None,
                )
                for element in pillar["elements"]:
                    update_metadata(
                        ...
                        element.get("filepath", None),
                        None,
                        element.get("normalized_layer", None),
                        None,
                        -1,
                        1,
                        None,
                    ...
                    for metric in element["metrics"]:
                        ...
                        update_metadata(
                            metric["metric_name"],
                            metric.get("filepath", None),
                            metric.get("raw_layer", None),
                            metric.get("normalized_layer", None),
                            None,
                            min,
                            max,
                            data_units,
    ...
    def check_config(self) -> bool:
        ...
        def check_region(region) -> bool:
            return (
                ...
                and region.keys() <= PillarConfig.REGION_KEYS
                and isinstance(RegionName(region["region_name"]), RegionName)
                and isinstance(region["pillars"], list)
                and isinstance(region["raw_data"], bool)
                and isinstance(region["translated_data"], bool)
                and isinstance(region["future_data"], bool)
                and isinstance(region["region_geoserver_name"], str)
                and all([check_pillar(pillar) for pillar in region["pillars"]])
            )

        def check_pillar(pillar) -> bool:
            return (
                isinstance(pillar, dict)
                and pillar.keys() <= PillarConfig.PILLAR_KEYS
                and isinstance(pillar["pillar_name"], str)
                and isinstance(pillar["elements"], list)
                and all([check_element(element) for element in pillar["elements"]])
            )

        def check_element(element) -> bool:
            return (
                isinstance(element, dict)
                and element.keys() <= PillarConfig.ELEMENT_KEYS
                and isinstance(element["element_name"], str)
                and isinstance(element["metrics"], list)
                and all([check_metric(metric) for metric in element["metrics"]])
            )

        def check_metric(metric) -> bool:
            return (
                isinstance(metric, dict)
                and metric.keys() <= PillarConfig.METRIC_KEYS
                and isinstance(metric["metric_name"], str)
            )
    ...
    def get_metric(
        self, region_name: str, pillar_name: str, element_name: str, metric_name: str
    ) -> Optional[Metric]:
