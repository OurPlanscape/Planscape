    ...
    def check_config(self) -> bool:
        ...
        def check_region(region) -> bool:
            return (
                ...
                and region.keys() <= set(["region_name", "display_name", "boundaries"])
                and isinstance(RegionName(region["region_name"]), RegionName)
                and isinstance(region["display_name"], str)
                and isinstance(region["display_name"], str)
                and isinstance(region["boundaries"], list)
                and all([check_boundary(boundary) for boundary in region["boundaries"]])
            )
        ...
        def check_boundary(boundary) -> bool:
            return (
                ...
                and isinstance(boundary["boundary_name"], str)
                and isinstance(boundary.get("display_name", ""), str)
                and isinstance(boundary["vector_name"], str)
                and isinstance(boundary["region_name"], str)
                and isinstance(boundary["filepath"], str)
                and isinstance(boundary["source_srs"], int)
                and isinstance(GeometryType(boundary["geometry_type"]), GeometryType)
                and isinstance(boundary["shape_name"], str)
            )
