...
class CreatePlanTest(TransactionTestCase):
    ...
    def test_missing_user(self):
        response = self.client.post(
            reverse("plan:create"), {"name": "plan"}, content_type="application/json"
        )
    ...
    def test_missing_name(self):
        ...
        response = self.client.post(
            reverse("plan:create"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
def create_plan(
    owner: User | None, name: str, geometry: GEOSGeometry | None, scenarios: list[int]
):
    """
    ...
    for num_scenarios in scenarios:
        ...
        for _ in range(num_scenarios):
            scenario = Scenario.objects.create(owner=owner, plan=plan, project=project)
            scenario.save()
    return plan
...
class DeletePlanTest(TransactionTestCase):
    ...
    def test_delete_ownerless_plan(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, json.dumps({"id": [self.plan1.pk]}).encode())
    ...
    def test_delete_owned_plan(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, json.dumps({"id": [self.plan2.pk]}).encode())
        self.assertEqual(Plan.objects.count(), 2)
        self.assertEqual(Project.objects.count(), 3)
    ...
    def test_delete_multiple_plans_fails_if_any_not_owner(self):
        ...
        response = self.client.post(
            reverse("plan:delete"), {"id": plan_ids}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(Plan.objects.count(), 3)
        self.assertEqual(Project.objects.count(), 4)
    ...
    def test_delete_multiple_plans(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, json.dumps({"id": plan_ids}).encode())
        self.assertEqual(Plan.objects.count(), 1)
...
class GetPlanTest(TransactionTestCase):
    ...
    def test_get_nonexistent_plan(self):
        self.client.force_login(self.user)
        response = self.client.get(
            reverse("plan:get_plan"), {"id": 25}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_plan_bad_stored_region(self):
        ...
        plan.save()
        response = self.client.get(
            reverse("plan:get_plan"), {"id": plan.pk}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class ListPlansTest(TransactionTestCase):
    ...
    def test_list_plans_by_owner_no_user(self):
        response = self.client.get(
            reverse("plan:list_plans_by_owner"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class CreateProjectTest(TransactionTestCase):
    ...
    def test_missing_user(self):
        response = self.client.post(
            reverse("plan:create_project"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class UpdateProjectTest(TransactionTestCase):
    def setUp(self):
        ...
        self.condition2_raw = Condition.objects.create(
            condition_dataset=self.base_condition2,
            condition_score_type=ConditionScoreType.CURRENT,
            is_raw=True,
        )
    ...
    def test_add_priority(self):
        ...
        response = self.client.put(
            reverse("plan:update_project"),
            {"id": self.project_with_user.pk, "priorities": ["condition2"]},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        project = Project.objects.get(id=self.project_with_user.pk)
    ...
    def test_patch_remove_priority(self):
        ...
        response = self.client.patch(
            reverse("plan:update_project"),
            {"id": self.project_with_user.pk, "priorities": []},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        project = Project.objects.get(id=self.project_with_user.pk)
...
class DeleteProjectsTest(TransactionTestCase):
    ...
    def test_delete_ownerless_project(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content, json.dumps([self.project_with_no_user.pk]).encode()
        )
    ...
    def test_delete_owned_project(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content, json.dumps([self.project_with_user.pk]).encode()
        )
        self.assertEqual(Project.objects.count(), 2)
    ...
    def test_delete_multiple_projects(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, json.dumps(project_ids).encode())
        self.assertEqual(Project.objects.count(), 1)
...
class GetProjectTest(TransactionTestCase):
    def setUp(self):
        ...
        self.plan_with_user = create_plan(self.user, "ownerless", stored_geometry, [])
    ...
    def test_get_nonexistent_project(self):
        self.client.force_login(self.user)
        response = self.client.get(
            reverse("plan:get_project"), {"id": 10}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class ListProjectsTest(TransactionTestCase):
    def setUp(self):
        ...
        self.plan_with_user = create_plan(self.user, "plan", stored_geometry, [])
        self.plan_without_user = create_plan(None, "plan", stored_geometry, [])

    def test_list_no_plan_id(self):
        self.client.force_login(self.user)
        response = self.client.get(
            reverse("plan:list_projects_for_plan"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class GetProjectAreaTest(TransactionTestCase):
    def setUp(self):
        ...
        self.project_area_no_user = ProjectArea.objects.create(
            owner=None,
            project=self.project_no_user,
            project_area=stored_geometry,
            estimated_area_treated=100,
        )
    ...
    def test_get_projectareas_no_project_id(self):
        response = self.client.get(
            reverse("plan:get_project_areas"), {}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_projectareas_no_user(self):
        ...
        self.assertEqual(response.json()[project_area_id]["properties"]["owner"], None)
        self.assertEqual(
            response.json()[project_area_id]["properties"]["project"],
            self.project_no_user.pk,
        )
        self.assertEqual(
            response.json()[project_area_id]["properties"]["estimated_area_treated"],
            100,
        )
        self.assertEqual(response.json()[project_area_id]["geometry"], self.geometry)
...
class GetScoresTest(TransactionTestCase):
    ...
    def _create_condition_db(
        self, condition_name: str, condition_raster_name: str
    ) -> int:
        base_condition = BaseCondition.objects.create(
...
class CreateScenarioTest(TransactionTestCase):
    def setUp(self):
        ...
        self.plan = create_plan(self.user, "plan", self.stored_geometry, [])
    ...
    def test_create_scenario_for_not_owned_plan(self):
        self.client.force_login(self.user)
        not_my_plan = create_plan(None, "not_my_plan", self.stored_geometry, [])
        response = self.client.post(
            reverse("plan:create_scenario"),
    ...
    def test_create_scenario_with_weights(self):
        ...
        weighted_pris = ScenarioWeightedPriority.objects.filter(scenario=scenario.pk)
...
class UpdateScenarioTest(TransactionTestCase):
    def setUp(self):
        ...
        self.plan = create_plan(self.user, "plan", stored_geometry, [])
        self.project = Project.objects.create(
            owner=self.user, plan=self.plan, max_budget=100
        )
        self.project_area = ProjectArea.objects.create(
    ...
    def test_create_scenario_invalid_id(self):
        self.client.force_login(self.user)
        response = self.client.patch(
            reverse("plan:update_scenario"), {"id": 10}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class GetScenarioTest(TransactionTestCase):
    ...
    def test_get_nonexistent_scenario(self):
        response = self.client.get(
            reverse("plan:get_scenario"), {"id": 10}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)

    def test_get_scenario_does_not_belong_to_user(self):
        plan_no_user = create_plan(None, "plan", None, [])
        not_owned_scenario = Scenario.objects.create(owner=None, plan=plan_no_user)
        self.client.force_login(self.user)
        response = self.client.get(
    ...
    def test_get_scenario_ok(self):
        ...
        self.assertEqual(scenario["priorities"], {"cond1": 2, "cond2": 3})
        self.assertEqual(
            scenario["project_areas"][str(self.project_area.pk)]["geometry"],
            self.geometry,
        )
        self.assertEqual(
            scenario["project_areas"][str(self.project_area.pk)]["properties"][
                "estimated_area_treated"
            ],
            200,
        )
...
class ListScenariosTest(TransactionTestCase):
    ...
    def test_list_scenario_ok(self):
        ...
        self.assertEqual(scenario1["priorities"], {"cond1": 2, "cond2": 3})
        self.assertEqual(
            scenario1["project_areas"][str(self.project_area.pk)]["geometry"],
            self.geometry,
        )
        self.assertEqual(
            scenario1["project_areas"][str(self.project_area.pk)]["properties"][
                "estimated_area_treated"
            ],
            200,
        )
        ...
        self.assertEqual(scenario2["priorities"], {"cond1": 4, "cond2": 5})
        self.assertEqual(
            scenario2["project_areas"][str(self.project_area.pk)]["geometry"],
            self.geometry,
        )
        self.assertEqual(
            scenario2["project_areas"][str(self.project_area.pk)]["properties"][
                "estimated_area_treated"
            ],
            200,
        )
...
class DeleteScenariosTest(TransactionTestCase):
    def setUp(self):
        ...
        self.scenario_with_no_user = Scenario.objects.create(
            owner=None,
            plan=self.plan_with_no_user,
            project=self.project_with_no_user,
            notes="my note2",
        )
    ...
    def test_delete_ownerless_scenario(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content, json.dumps([self.scenario_with_no_user.pk]).encode()
        )
    ...
    def test_delete_owned_scenario(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content, json.dumps([self.scenario_with_user.pk]).encode()
        )
        self.assertEqual(Scenario.objects.count(), 2)
    ...
    def test_delete_multiple_scenarios(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, json.dumps(scenario_ids).encode())
...
class BulkCreateProjectAreaTest(TransactionTestCase):
    ...
    def test_good_polygon(self):
        ...
        self.assertEqual(area.project_area.coords, self.stored_geometry1.coords)

        area = ProjectArea.objects.get(id=(response.json()[1]))
        self.assertEqual(area.owner, self.user)
        self.assertEqual(area.project, self.project)
        self.assertEqual(area.project_area.coords, self.stored_geometry2.coords)
    ...
    def test_good_multipolygon(self):
        ...
        response = self.client.post(
            reverse("plan:create_project_areas_for_project"),
            {"project_id": self.project.pk, "geometries": self.multipolygon_geometries},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(ProjectArea.objects.count(), 2)
