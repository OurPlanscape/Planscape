...
def get_scenario_by_id(user: User, id_url_param: str, params: QueryDict) -> Scenario:
    assert isinstance(params[id_url_param], str)
    scenario_id = params.get(id_url_param, "0")
    scenario = Scenario.objects.select_related("project").get(id=scenario_id)

    if scenario.owner != user:
        raise ValueError("You do not have permission to view this scenario.")
    return scenario
...
def get_plan_by_id(user, id_url_param: str, params: QueryDict):
    ...
    plan = (
        Plan.objects.annotate(projects=Count("project", distinct=True))
...
def get_project_by_id(user: User, id_url_param: str, params: QueryDict) -> Project:
    assert isinstance(params[id_url_param], str)
    ...
    try:
        ...
        for plan in plans:
            ...
            if owner_id != plan_owner_id:
                raise ValueError("Cannot delete plan; plan is not owned by user")
        for plan in plans:
            plan.delete()
...
def _serialize_plan(plan: Plan, add_geometry: bool) -> dict:
    ...
    if "creation_time" in result:
        result["creation_timestamp"] = round(
            datetime.datetime.fromisoformat(
                result["creation_time"].replace("Z", "+00:00")
            ).timestamp()
        )
...
def get_plan(request: HttpRequest) -> HttpResponse:
    try:
        ...
        return JsonResponse(
            _serialize_plan(get_plan_by_id(user, "id", request.GET), True)
        )
    except Exception as e:
...
def list_plans_by_owner(request: HttpRequest) -> HttpResponse:
    try:
        ...
        plans = (
            Plan.objects.filter(owner=owner_id)
            .annotate(projects=Count("project", distinct=True))
            .annotate(scenarios=Count("project__scenario"))
        )
        return JsonResponse(
            [_serialize_plan(plan, False) for plan in plans], safe=False
        )
    except Exception as e:
        return HttpResponseBadRequest("Ill-formed request: " + str(e))
...
def _validate_constraint_values(
    ...
    if max_treatment_area_ratio is not None and (
        not (isinstance(max_treatment_area_ratio, (int, float)))
        or max_treatment_area_ratio < 0
    ):
        raise ValueError("Max treatment must be a number value >= 0.0")
    ...
    if max_slope is not None and (
        ...
        raise ValueError("Max slope must be a number value >= 0.0")


def _set_project_parameters(
    max_budget, max_treatment_area_ratio, max_road_distance, max_slope, project: Project
):
    project.max_budget = float(max_budget) if max_budget else None
    project.max_treatment_area_ratio = (
        float(max_treatment_area_ratio) if max_treatment_area_ratio else None
    )
...
def _set_priorities(priorities, project: Project):
    if priorities is not None:
        for i in range(len(priorities)):
            base_condition = BaseCondition.objects.get(condition_name=priorities[i])
...
def create_project(request: HttpRequest) -> HttpResponse:
    try:
        ...
        if not (
            (owner is None and plan.owner is None)
            or (
                owner is not None
                and plan.owner is not None
                and owner.pk == plan.owner.pk
            )
        ):
            raise ValueError("Cannot create project; plan is not owned by user")
        ...
        _set_project_parameters(
            max_budget, max_treatment_area_ratio, max_road_distance, max_slope, project
        )
...
def update_project(request: HttpRequest) -> HttpResponse:
    try:
        ...
        if request.method == "PUT":
            ...
            raise KeyError("HTTP methods other than PUT are not yet implemented")
        project.save()
        return HttpResponse(str(project.pk))
    except Exception as e:
        return HttpResponseBadRequest("Ill-formed request: " + str(e))
...
def list_projects_for_plan(request: HttpRequest) -> HttpResponse:
    try:
        ...
        return JsonResponse(
            [_serialize_project(project) for project in projects], safe=False
        )
    except Exception as e:
        return HttpResponseBadRequest("Ill-formed request: " + str(e))
...
def _serialize_project(project: Project) -> dict:
    ...
    if "priorities" in result:
        result["priorities"] = [
            Condition.objects.get(pk=priority).condition_dataset.condition_name
...
def create_project_area(request: HttpRequest) -> HttpResponse:
    try:
        ...
        if not (
            (owner is None and project.owner is None)
            or (
                owner is not None
                and project.owner is not None
                and owner.pk == project.owner.pk
            )
        ):
            raise ValueError("Cannot create project; plan is not owned by user")
...
def create_project_areas_for_project(request: HttpRequest) -> HttpResponse:
    try:
        ...
        if not (
            ...
            raise ValueError("Cannot create project area; project is not owned by user")
        ...
        project_areas = ProjectArea.objects.bulk_create(
            [
                ProjectArea(**{"owner": owner, "project": project, "project_area": p})
                for p in polygons
            ]
        )
...
def _create_scenario(request: HttpRequest):
    ...
    if not (
        (owner is None and plan.owner is None)
        or (owner is not None and plan.owner is not None and owner.pk == plan.owner.pk)
    ):
        raise ValueError("Cannot create scenario; plan is not owned by user")
    ...
    if not (
        ...
        raise ValueError("Cannot create scenario; project is not owned by user")
    ...
    if weights is None:
        raise ValueError("Scenario must have weights for priorites")
    if len(priorities) != len(weights):
        raise ValueError("Each priority must have a single assigned weight")
...
def update_scenario(request: HttpRequest) -> HttpResponse:
    try:
        ...
        if scenario.owner != owner:
            raise ValueError("You do not have permission to update this scenario.")
        ...
        if request.method == "PATCH":
            ...
            raise KeyError("HTTP methods other than PATCH are not yet implemented")
        scenario.save()
...
def _serialize_scenario(
    scenario: Scenario, weights: QuerySet, areas: QuerySet, project: Project
) -> dict:
        ...
        if "priority" in serialized_weight and "weight" in serialized_weight:
            result["priorities"][
                Condition.objects.get(
                    pk=serialized_weight["priority"]
                ).condition_dataset.condition_name
...
def get_scenario(request: HttpRequest) -> HttpResponse:
    try:
        ...
        return JsonResponse(
            _serialize_scenario(scenario, weights, project_areas, scenario.project),
            safe=False,
        )
    except Exception as e:
        return HttpResponseBadRequest("Ill-formed request: " + str(e))
...
def list_scenarios_for_plan(request: HttpRequest) -> HttpResponse:
    try:
        ...
        scenarios = Scenario.objects.select_related("project").filter(
            owner=user, plan=plan_id
        )
...
def delete_scenarios(request: HttpRequest) -> HttpResponse:
    try:
        ...
        scenarios = [
            Scenario.objects.get(pk=scenario_id) for scenario_id in scenario_ids
        ]
...
def favorite_scenario(request: HttpRequest) -> HttpResponse:
    try:
        ...
        user = get_user(request)
        if scenario.owner != user:
            raise ValueError("You do not have permission to favorite this scenario.")
...
def unfavorite_scenario(request: HttpRequest) -> HttpResponse:
    try:
        ...
        user = get_user(request)
        if scenario.owner != user:
            raise ValueError("You do not have permission to unfavorite this scenario.")
...
def queue_forsys_lambda_prototype(request: HttpRequest) -> HttpResponse:
    try:
        ...
        return JsonResponse({"statusCode": 200, "messageId": response["MessageId"]})
