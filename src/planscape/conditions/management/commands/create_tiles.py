...
class Command(BaseCommand):
    help = (
        "Converts a directory of rasters <path>/data/... specified in "
        "config/conditions.json into tile in <path/tiles/..."
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "data_directory", nargs="?", type=str, help="Location of the raster data."
        )
        parser.add_argument(
            "--zoom_levels",
            nargs="?",
        ...
        parser.add_argument(
            ...
            type=str,
            default="turbo",
            choices=["turbo", "viridis"],
            help="Colormap used for the tiles.",
        )
        parser.add_argument(
            "--force",
            type=bool,
    ...
    def _convert_tif_to_tiles(
        ...
        raster_file = raster_base + ("_normalized.tif" if normalized else ".tif")
        input_dir = os.path.dirname(os.path.join(data_directory, filepath))
        input_file = os.path.join(input_dir, raster_file)
        colormap_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            colormap + ("_inverted.cpt" if invert else ".cpt"),
        )
        if not os.path.exists(colormap_file):
            raise CommandError("Unknown colormap file {0}.".format(colormap_file))

        # Set up the outputs
        output_dir = os.path.dirname(
            os.path.join(data_directory.replace("data", "tiles"), filepath)
        )
        gdaldem_file = os.path.join(output_dir, raster_file.replace(".tif", ".vrt"))
        output_file = os.path.join(output_dir, raster_file.removesuffix(".tif"))
        ...
        if not dry_run:
            ...
            subprocess.run(
                gdaldem, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True
            )
            ...
            for element in config.get_elements(pillar):
                ...
                for metric in config.get_metrics(element):
                    ...
                    self._convert_tif_to_tiles(
                        ...
                        metric.get("filepath"),
                        normalized=False,
                        invert=metric.get("invert_raw", False),
                        colormap=colormap,
                        zoom_levels=zoom_levels,
                        force=force,
                    )
