        ...
        for element in config.get_elements(pillar):
            for metric in config.get_metrics(element):
                ...
                for metric_type in [".tif", "_normalized.tif"]:
                    ...
                    base_filepath = (
                        os.path.join(os.path.dirname(data_path), metric_filepath)
                        + metric_type
                    )
                    with rasterio.open(
                        base_filepath, "w", nan_condition.profile
                    ) as dst:
...
def _load_metric(metric: Metric, metric_type: str, base_metric: BaseCondition):
    ...
    metric_path = (
        os.path.join(
            os.path.dirname(os.path.join(settings.BASE_DIR, "../..")), metric_filepath
        )
        + metric_type
    )

    cmds = (
        "export PGPASSWORD=pass; raster2pgsql -s 9822 -a -I -C -Y -f raster -n name -t 256x256 "
        + metric_path
        + " public.conditions_conditionraster | psql -U planscape -d planscape -h localhost -p 5432"
    )
...
def _load_condition(
    ...
    if len(condition_query) > 0:
        print("Condition " + condition_name + " already exists; deleting.")
        condition_query.delete()
    condition = Condition(
        condition_dataset=base_condition,
        raster_name=raster_name,
        condition_score_type=condition_score_type,
        is_raw=False,
    )
    condition.save()
    print("Saved Condition: " + condition.raster_name)
    ...
    for pillar in pillars:
        ...
        for element in config.get_elements(pillar):
            ...
            filepath = os.path.join(
                os.path.dirname(os.path.join(settings.BASE_DIR, "../..")), raster_path
            )

            # Save locally
            if save and computed_element is not None:
                _save_condition(
                    computed_element.raster, filepath, computed_element.profile
                )
            ...
            _load_condition(
                ...
                ConditionLevel.ELEMENT,
                ConditionScoreType.CURRENT,
...
def compute_pillars(region_name: str, save: bool, reload: bool):
    ...
    for pillar in pillars:
        ...
        if save and computed_pillar is not None:
            _save_condition(computed_pillar.raster, filepath, computed_pillar.profile)
