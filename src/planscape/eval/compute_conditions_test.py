...
class FakeConditionReader(cc.ConditionReader):
    def read(self, filepath: str, condition_type: ConditionScoreType) -> Optional[Tuple[ConditionMatrix, str]]:
...
class ScoreMetricTest(unittest.TestCase):
    def test_score_metric_no_file_returns_none(self):
        ...
        condition, profile = cc.score_metric(
...
class ScoreElementTest(unittest.TestCase):
    def test_score_element_no_file_returns_none(self):
        ...
        computed_element, profile = cc.score_element(
    ...
    def test_score_element_with_file_returns_not_none(self):
        element: Element = {'element_name': 'element',
                            'metrics': [], 'filepath': 'path/to/file'}
        self.assertIsNotNone(cc.score_element(
            FakeConditionReader(), element, ConditionScoreType.IMPACT))
    ...
    def test_recompute_element_one_metric(self):
        ...
        print(computed_element)
        if computed_element is not None:
            self.assertTrue(np.all(computed_element == np.array([[1, 20, 3], [4, 5, 6]])))
    ...
    def test_recompute_element_two_metrics_mean(self):
        ...
        self.assertIsNotNone(computed_element)
        if computed_element is not None:
            self.assertTrue(np.all(computed_element == np.array([[0, 11, 3], [0, 5, 6]])))
    ...
    def test_recompute_element_two_metrics_min(self):
        ...
        if computed_element is not None:
            self.assertTrue(
                np.all(computed_element == np.array([[-1, 2, 3], [-4, 5, 6]])))
...
class ScorePillarTest(unittest.TestCase):
    def test_score_pillar_no_file_returns_none(self):
        ...
        self.assertIsNone(cc.score_pillar(
            FakeConditionReader(), pillar, ConditionScoreType.ADAPT))

    def test_score_pillar_with_file_returns_not_none(self):
        pillar: Pillar = {'pillar_name': 'pillar',
                          'elements': [], 'filepath': 'path/to/file'}
        self.assertIsNotNone(cc.score_pillar(
            FakeConditionReader(), pillar, ConditionScoreType.IMPACT))
    ...
    def test_recompute_pillar_two_metrics_mean(self):
        ...
        metric2: Metric = {'metric_name': 'metric', 'filepath': 'special'}
        element: Element = {'element_name': 'element',
                            'metrics': [metric1, metric2], 'operation': 'MEAN'}
        pillar: Pillar = {'pillar_name': 'pillar', 'elements': [element]}
        score = cc.score_pillar(
            FakeConditionReader(), pillar, ConditionScoreType.IMPACT, recompute=True)
        self.assertIsNotNone(score)
    ...
    def test_recompute_pillar_fallback_to_element_two_elements(self):
        ...
        element1: Element = {'element_name': 'element',
                             'metrics': [metric1, metric2]}
        element2: Element = {'element_name': 'element',
                             'metrics': [], 'filepath': 'path/to/element'}
        pillar: Pillar = {'pillar_name': 'pillar', 'elements': [
            element1, element2], 'operation': 'MEAN'}
        score = cc.score_pillar(
            FakeConditionReader(), pillar, ConditionScoreType.IMPACT, recompute=True)
        self.assertIsNotNone(score)
        if score is not None:
...
class ScoreConditionTest(unittest.TestCase):
    class FakePillarConfig(pc.PillarConfig):
        def __init__(self, filename: str):
            self._metric1: Metric = {
                'metric_name': 'metric1', 'filepath': 'special'}
            self._metric2: Metric = {'metric_name': 'metric2',
                                     'filepath': 'path/to/metric'}
            self._element1: Element = {'element_name': 'element1',
                                       'metrics': [self._metric1,   self._metric2]}
            self._element2: Element = {'element_name': 'element2',
                                       'metrics': [], 'filepath': 'path/to/element'}
            self._pillar: Pillar = {'pillar_name': 'pillar', 'elements': [
                self._element1,   self._element2], 'operation': 'MEAN', 'filepath': 'path/to/pillar'}
    ...
    def test_score_condition_unknown_pillar_returns_none(self):
        self.assertIsNone(cc.score_condition(self.FakePillarConfig(""),
                                             FakeConditionReader(), "region/pillar1", ConditionScoreType.ADAPT))
    ...
    def test_score_condition_known_pillar_returns_value(self):
        ...
        score = cc.score_condition(pillar_config,
                                   FakeConditionReader(), "region/pillar", ConditionScoreType.ADAPT)
        self.assertIsNotNone(score)
    ...
    def test_score_condition_unknown_element_returns_none(self):
        self.assertIsNone(cc.score_condition(self.FakePillarConfig(""),
                                             FakeConditionReader(), "region/pillar/element", ConditionScoreType.ADAPT))

    def test_score_condition_known_element_returns_value(self):
        pillar_config = self.FakePillarConfig("")
        element = pillar_config.get_element("region", "pillar", "element1")
        score,profile = cc.score_condition(pillar_config,
                                   FakeConditionReader(), "region/pillar/element1", ConditionScoreType.IMPACT, recompute=True)
        self.assertIsNotNone(score)
        self.assertIsNotNone(element)
        if element is not None:
            expected,profile = cc.score_element(
                FakeConditionReader(), element, ConditionScoreType.IMPACT, recompute=True)
            self.assertTrue(np.all(score == expected))

    def test_score_condition_unknown_metric_returns_none(self):
        self.assertIsNone(cc.score_condition(self.FakePillarConfig(""),
                                             FakeConditionReader(), "region/pillar/element1/metric", ConditionScoreType.ADAPT))
    ...
    def test_score_condition_known_metric_returns_value(self):
        ...
        if metric is not None:
            ...
            print(score)
            print(condition)
            self.assertTrue(np.all(score == condition))
