...
class ModuleAPITests(TestCase):
    ...
    @mock.patch("modules.base.get_module")
    def test_get_details_success_200(self, get_module_mock):
        pk = "forsys"
        payload = {"name": "forsys", "version": "1.0.0"}
        get_module_mock.return_value = payload
    ...
    @mock.patch("modules.base.get_module")
    def test_get_details_success_200_with_geometry(self, get_module_mock):
        pk = "forsys"
        payload = {"name": "forsys", "version": "1.0.0"}
        get_module_mock.return_value = payload
        geometry = {
        ...
        url = f"{reverse('api:modules:modules-details', kwargs={'pk': pk})}?{urlencode(query_params)}"
        resp = self.client.get(url)
    ...
    @mock.patch("modules.base.get_module", side_effect=KeyError)
    def test_get_details_404_when_service_raises_keyerror(self, _get_module_mock):
        pk = "does-not-exist"
        url = reverse("api:modules:modules-details", kwargs={"pk": pk})
        resp = self.client.get(url)
