...
create or replace function get_rast_tile(
	  ...
	  param_colormap text,
	  param_schema text, param_table text) returns bytea
    immutable
    parallel safe
    cost 1000
    language plpgsql
...
BEGIN
    ...
    USING param_schema, param_table, 'rast',
    ...
    */
    var_sql :=
        'WITH r AS (SELECT ST_Clip(rast, ST_Transform($3, $5)) As rast FROM ' ||
                    quote_ident(param_schema) || '.' || quote_ident(param_table) ||
        '           WHERE ST_Intersects(rast, ST_Transform($1, $5)) limit 15)
        SELECT ST_Clip(ST_Union(rast), $1) As rast
        FROM (SELECT ST_Resample(ST_Transform(rast, $4), $2, true,''Billinear'') As rast FROM r) As final';
