...
def validate_scenario_for_run(scenario: Scenario) -> List[str]:
    errors: List[str] = []

    if not scenario.treatment_goal:
        errors.append("Scenario has no treatment_goal.")

    cfg = scenario.configuration or {}
    has_budget = cfg.get("max_budget") is not None
    has_area = cfg.get("max_area") is not None
    if has_budget and has_area:
        errors.append("Provide only one of `max_budget` or `max_area`.")
    if not has_budget and not has_area:
        errors.append("Provide one of `max_budget` or `max_area`.")

    if not scenario.planning_area or not scenario.planning_area.geometry:
        errors.append("Scenario planning_area or its geometry is missing.")

    if (
        scenario.treatment_goal
        and scenario.planning_area
        and scenario.planning_area.geometry
    ):
        try:
            coverage = scenario.treatment_goal.get_coverage()
            if coverage and not (
                coverage.contains(scenario.planning_area.geometry)
                or coverage.intersects(scenario.planning_area.geometry)
            ):
                errors.append("Planning area is outside Treatment Goal coverage.")
        except Exception:
            errors.append("Could not validate Treatment Goal coverage.")

    stands_qs = scenario.planning_area.get_stands(stand_size=scenario.get_stand_size())
    if not stands_qs.exists():
        errors.append(
            "No stands available for the selected stand_size in this planning area."
        )

    if scenario.result_status == ScenarioResultStatus.RUNNING:
        errors.append("Scenario is already running.")
