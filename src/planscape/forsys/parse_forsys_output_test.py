...
class TestForsysRankingOutputForMultipleScenarios(TestCase):
    def test_parses_output(self) -> None:
        ...
        parsed_output = ForsysRankingOutputForMultipleScenarios(
            raw_forsys_output, ["p1", "p2"], None, None, "proj_id", "area", "cost"
        )
            ...
            scenarios,
            {
                "p1:1 p2:1": {
                    ...
                    "ranked_projects": [
                        {
                            "id": 1,
                            "weighted_priority_scores": {"p1": 0.5, "p2": 0.1},
                            "total_score": 0.6,
                            "rank": 1,
                        },
                        {
                            "id": 2,
                            "weighted_priority_scores": {"p1": 0.1, "p2": 0.4},
                            "total_score": 0.5,
                            "rank": 2,
                        },
                        {
                            "id": 3,
                            "weighted_priority_scores": {"p1": 0.3, "p2": 0.1},
                            "total_score": 0.4,
                            "rank": 3,
                        },
                    ],
                ...
                "p1:1 p2:2": {
                    ...
                    "ranked_projects": [
                        {
                            "id": 2,
                            "weighted_priority_scores": {"p1": 0.1, "p2": 0.8},
                            "total_score": 0.9,
                            "rank": 1,
                        },
                        {
                            "id": 1,
                            "weighted_priority_scores": {"p1": 0.5, "p2": 0.2},
                            "total_score": 0.7,
                            "rank": 2,
                        },
                        {
                            "id": 3,
                            "weighted_priority_scores": {"p1": 0.3, "p2": 0.2},
                            "total_score": 0.5,
                            "rank": 3,
                        },
                    ],
    ...
    def test_fails_if_priority_ordering_is_wrong(self) -> None:
        ...
        with self.assertRaises(Exception) as context:
            ...
            ForsysRankingOutputForMultipleScenarios(
                raw_forsys_output, ["p2", "p1"], None, None, "proj_id", "area", "cost"
            )

        self.assertEqual(
            str(context.exception), "header, Pr_1_p2, is not a forsys output header"
        )
    ...
    def test_fails_if_proj_id_header_is_wrong(self) -> None:
        ...
        self.assertEqual(
            str(context.exception), "header, project_id, is not a forsys output header"
        )
    ...
    def test_fails_if_cost_header_is_wrong(self) -> None:
        ...
        with self.assertRaises(Exception) as context:
            ...
            ForsysRankingOutputForMultipleScenarios(
                raw_forsys_output, ["p1", "p2"], None, None, "proj_id", "area", "c"
            )

        self.assertEqual(
            str(context.exception), "header, ETrt_c, is not a forsys output header"
        )

    def test_limits_area(self) -> None:
        raw_forsys_output = self._get_parsed_forsys_output()
        parsed_output = ForsysRankingOutputForMultipleScenarios(
            raw_forsys_output, ["p1", "p2"], 25, None, "proj_id", "area", "cost"
        )
    ...
    def test_limits_area_by_skipping_top_project(self) -> None:
        raw_forsys_output = self._get_parsed_forsys_output()
        parsed_output = ForsysRankingOutputForMultipleScenarios(
            raw_forsys_output, ["p1", "p2"], 10, None, "proj_id", "area", "cost"
        )
    ...
    def test_limits_cost(self) -> None:
        raw_forsys_output = self._get_parsed_forsys_output()
        parsed_output = ForsysRankingOutputForMultipleScenarios(
            raw_forsys_output, ["p1", "p2"], None, 1200, "proj_id", "area", "cost"
        )
    ...
    def test_limits_cost_by_skipping_top_project(self) -> None:
        raw_forsys_output = self._get_parsed_forsys_output()
        parsed_output = ForsysRankingOutputForMultipleScenarios(
            raw_forsys_output, ["p1", "p2"], None, 550, "proj_id", "area", "cost"
        )
...
class TestForsysRankingOutputForASingleScenario(TestCase):
    def test_parses_output(self) -> None:
        parsed_output = ForsysRankingOutputForASingleScenario(
            self._get_parsed_forsys_output(),
            {"p1": 1, "p2": 2},
            None,
            None,
            "proj_id",
    ...
    def test_fails_given_irrelevant_priority_weights(self) -> None:
        ...
        self.assertEqual(
            str(context.exception), "header, ETrt_p3, is not a forsys output header"
        )
...
class TestForsysGenerationOutputForASingleScenario(MergePolygonsTest):
                ...
                "ranked_projects": [
                    {
                        ...
                        "total_score": 0.9,
                        "rank": 1,
                        "geo_wkt": self._create_polygon_in_default_crs(
                            ((1, 1), (0, 1), (0, -1), (1, -1), (1, 1))
                        ).wkt,
                    },
                    {
    ...
    def test_fails_if_geo_wkt_header_is_wrong(self) -> None:
        ...
        self.assertEqual(
            str(context.exception), "header, geometry, is not a forsys output header"
        )

    def _create_polygon_in_default_crs(
        self, coordinates: tuple[tuple[int, int]]
    ) -> MultiPolygon:
        geo = MergePolygonsTest._create_polygon(self, coordinates)
        geo.transform(
            CoordTransform(
    ...
    def _get_parsed_forsys_output(self) -> dict:
        ...
        raw_forsys_output = ro.vectors.ListVector(
            {
                "stand_output": _convert_dictionary_of_lists_to_rdf(self, stand_output),
