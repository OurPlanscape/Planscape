...
class RasterMergerTest(TestCase):
    def test_initializes_valid_raster(self) -> None:
        ...
        self._assert_raster_equals(
            merger.merged_raster, self._create_valid_raster())
    ...
    def test_fails_to_initialize_raster_with_no_bands(self) -> None:
        ...
        self.assertEqual(str(context.exception),
                         'invalid raster has 0 bands (expected 1)')
    ...
    def test_fails_to_initialize_raster_with_gt1_bands(self) -> None:
        ...
        with self.assertRaises(Exception) as context:
            RasterMerger(raster)
        self.assertEqual(str(context.exception),
                         'invalid raster has 2 bands (expected 1)')
    ...
    def test_merges_rasters_in_place(self):
        ...
        self._assert_raster_equals(
            merger.merged_raster, expected_merged_raster)
    ...
    def test_merges_rasters_with_different_origins_x(self):
        ...
        expected_merged_raster = GDALRaster({
            ...
            'origin': [10, 15],
            'bands': [{
                'data': (np.nan, np.nan, np.nan, np.nan, 1, 2, 3, 4,
                         5, 6, 7, 8, np.nan, np.nan, np.nan, np.nan,
                         9, 10, 11, 12, np.nan, np.nan, np.nan, np.nan,
                         13, 14, 15, 16, np.nan, np.nan, np.nan, np.nan),
                'nodata_value': np.nan
            }]
        })
        self._assert_raster_equals(
