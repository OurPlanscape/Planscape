...
CREATE OR REPLACE FUNCTION generate_stand_metrics(_condition_id INT, _size VARCHAR, _clean bool)
RETURNS VOID AS $$
DECLARE
    raster_geometry geometry;
    stand record;
...
BEGIN
    ...
    IF _clean THEN
        DELETE FROM 
            stands_standmetric sm
        USING
            stands_stand ss
        WHERE
            ss.id = sm.stand_id AND
            sm.condition_id = _condition AND
            ss.size = _size
        WHERE condition_id = _condition_id;
    END IF;
