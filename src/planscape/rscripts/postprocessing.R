summarize_metrics <- function(forsys_output, stand_data, datalayers) {
  ...
  lookup <- setNames(pcp_fields, output_fields)
  ...
  summarized_metrics <- stand_output |> group_by(proj_id) |> summarize(across(all_of(pcp_fields), sum, .names = "sum_{col}")) |> rename(all_of(lookup))
  summarized_metrics
}
