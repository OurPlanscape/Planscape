...
import { NOTE_DELETE_DIALOG, SNACK_ERROR_CONFIG, SNACK_NOTICE_CONFIG } from '@app/shared';
...
@Component({
  ...
  standalone: true,
  imports: [ExpandedPanelComponent, ModalComponent, NgIf, NotesSidebarComponent],
...
export class NotesExpandedPanelComponent {
  ...
  hasPermission(plan: any) {
    console.log('todo: actually check', plan);
    return true;
  }
  ...
  handleNoteDelete(note: Note) {
    ...
    dialogRef
      ...
      .subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.notesService
            .deleteNote(this.plan.id, note.id)
            .subscribe({
              next: () => {
                this.snackbar.open(
                  `Deleted note`,
