...
describe('Step3Component', () => {
  ...
  it('should have invalid max_slope if value is less than 0', () => {
    const control = component.form.get('configuration.max_slope');
    control?.setValue(-1);
  ...
  it('should have invalid min_distance_from_road if value is less than 0', () => {
    const control = component.form.get('configuration.min_distance_from_road');
    control?.setValue(-10);
    expect(control?.valid).toBeFalse();
    expect(control?.hasError('min')).toBeTrue();
  });
  ...
  it('should have form invalid if one of the fields is invalid', () => {
    component.form.get('configuration.max_slope')?.setValue(50);
    component.form
      .get('configuration.min_distance_from_road')
      ?.setValue(100001);
    expect(component.form.valid).toBeFalse();
  });

  it('should have form valid if both fields are valid', () => {
    component.form.get('configuration.max_slope')?.setValue(10);
    component.form.get('configuration.min_distance_from_road')?.setValue(1000);
