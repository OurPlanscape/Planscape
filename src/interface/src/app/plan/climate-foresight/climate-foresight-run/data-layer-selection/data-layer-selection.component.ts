  ...
  implements OnChanges
{
  ...
  selectedCount$ = this.dataLayersState.addedCount$;

  canProceed$ = this.dataLayersState.hasAddedDatalayers$;

  canSelectMore$ = this.dataLayersState.canAddMoreDatalayers$;
  ...
  constructor(
    ...
    this.dataLayersState.addedDataLayer$
    ...
    if (this.run?.input_datalayers) {
      ...
      this.climateForesightService.getDataLayers().subscribe({
        next: (layers: DataLayer[]) => {
          ...
          this.dataLayersState.updateAddedLayers(dls);
        },
        error: (err: any) => {
  ...
  removeDataLayer(layer: DataLayer): void {
    this.dataLayersState.removeAddedLayer(layer);
  }
