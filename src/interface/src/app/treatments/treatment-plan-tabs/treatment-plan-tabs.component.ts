...
import {
  NotesSidebarComponent, NotesSidebarState
} from '@styleguide';
...
export class TreatmentPlanTabsComponent implements OnInit {
    ...
    dialogRef
      ...
      .subscribe((confirmed) => {
        if (confirmed && this.treatmentPlanId) {
          this.notesService.deleteNote(this.treatmentPlanId, note.id).subscribe({
            next: () => {
              this.snackbar.open(
                `Deleted note`,
                'Dismiss',
